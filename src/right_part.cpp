...
right_part::right_part(std::function<double()(double)>& _f) {
    f = _f;
    f2 = [&f](double x, double x0, double h) -> double {
        return f(x)*(x - x0)/h;
    };
    f3 = [&f](double x, double x0, double h) -> double {
...
Vector3D right_part::fact(double x0, double h) {
    integrate integral;
    std::function< double (double) > _f2 = [x0,h](double x) -> double {
        return f2(x, x0, h);
    };
    std::function< double (double) > _f3 = [x0,h](double x) -> double {
        return f3(x, x0, h);
    };
